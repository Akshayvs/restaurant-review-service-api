...
const restaurants = {
    ...
    getRestaurantsByName: function (baseRoute) {
        baseRoute.get('/byname', function (request, response) {
            ...
            if (query && /^\w+(,\w+)*$/.test(query)) {
                ...
                connectionPool.getConnection(function (error, connection) {
                    connection.query(`SELECT * FROM Restaurants INNER JOIN Addresses ON Restaurants.address=Addresses.address WHERE Restaurants.name IN (${names})`, function (error, result) {
                        if (error) {
    ...
    getRestaurantsByCity: function (baseRoute) {
        baseRoute.get('/bycity', function (request, response) {
            ...
            if (query && /^\w+(,\w+)*$/.test(query)) {
                ...
                connectionPool.getConnection(function (error, connection) {
                    connection.query(`SELECT * FROM Restaurants INNER JOIN Addresses ON Restaurants.address=Addresses.address WHERE Addresses.city IN (${city})`, function (error, result) {
                        if (error) {
                            handleError('Error while querying data from  Database', error, response);
                        }
                        connection.release();
    ...
    getRestaurantsByCategory: function (baseRoute) {
        baseRoute.get('/bycategory', function (request, response) {
            ...
            if (query && /^\w+(,\w+)*$/.test(query)) {
                ...
                connectionPool.getConnection(function (error, connection) {
                    connection.query(`SELECT * FROM Restaurants INNER JOIN Addresses ON Restaurants.address=Addresses.address WHERE Restaurants.category IN (${city})`, function (error, result) {
                        if (error) {
                            handleError('Error while querying data from  Database', error, response);
                        }
                        connection.release();
                        ...
                        if (e || result.length !== 0) {
                            ...
                            connection.query(`INSERT into Restaurants SET ?`, restaurantValues, function (e, res) {
                                ...
                                connection.query(`INSERT into Addresses SET ?`, addressValues, function (e) {
                                    ...
                                    connection.query(`SELECT * FROM Restaurants INNER JOIN Addresses ON Restaurants.address=Addresses.address WHERE Restaurants.address = '${address}'`, function (err, res) {
                                        if (e) {
                                            return handleError(' Error while Inserting in the Database', e, response);
                                        }
    ...
    updateRestaurant: function (baseRoute) {
        /*
        Input Validation for original value and new value
        Check if Restaurants table contains the input address (address is the primary key in Restaurants and Addresses tables)
        Update the values in Restaurants and Addresses tables.
        Fetch the newly saved value from the database and return it to the user as a confirmation
        */
            ...
            if (originalInputValidation === true) {
                if (newValueInputValidation === true) {
                    ...
                    connectionPool.getConnection(function (err, connection) {
                        //Check if Restaurants table contains the input address.
                        connection.query(`SELECT * from Restaurants WHERE address = '${original.address}'`, function (error, result) {
                            if (error) {
                                return handleError('error in connecting to database', '500', response);
                            } else if (result.length === 0) {
                                return handleError('Invalid Original Value', '200', response);
                            }
                            ...
                            connection.query(`Update Restaurants SET ? WHERE address = '${original.address}'`, restaurantUpdate, function (e) {
                                ...
                                connection.query(`Update Addresses SET ? WHERE address = '${original.address}'`, addressesUpdate, function (e) {
                                    ...
                                    connection.query(`SELECT * FROM Restaurants INNER JOIN Addresses ON Restaurants.address=Addresses.address WHERE Restaurants.address = '${newValue.address}'`, function (err, res) {
                                        if (e) {
                                            return handleError('Error while updating data', e, response);
                                        }
                                        return response.status(200).send(res);
